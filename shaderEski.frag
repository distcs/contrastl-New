...
void main() {
    ...
    // c += clamp(texture2D(img, sortedUV - randomOffset).rgb * nweFloat, 0.0, 1.0);
    // c -= clamp(texture2D(img, vec2(sortedUV.x, sortedUV.y) * sortedUV).rgb * nweFloat,0.0, 1.0);

    c += clamp(texture2D(img, sortedUV).rgb * nweFloat, 0.0, 1.0);
    c -= clamp(texture2D(img, vec2(sortedUV.x, sortedUV.y) * sortedUV).rgb * nweFloat, 0.0, 1.0);
    ...
    // c.rgb = ((c.rgb - vec3(0.499)) * 1.003 + vec3(0.501));
